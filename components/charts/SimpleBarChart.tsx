"use client";

import { Bar, BarChart, CartesianGrid, ResponsiveContainer, Tooltip, XAxis, YAxis } from "recharts";
import { fromMs, formatNumber } from "@/lib/format";
import { useLanguage } from "@/lib/i18n/context";

export function SimpleBarChart({ data }: { data: { day: string; sum: number }[] }) {
  const { locale, t } = useLanguage();
  const chartData = data.map((d) => ({ day: d.day.slice(8, 10), fullDay: d.day, sum: fromMs(d.sum) }));

  return (
    <ResponsiveContainer width="100%" height={220}>
      <BarChart data={chartData} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
        <CartesianGrid vertical={false} stroke="#eef1f8" />
        <XAxis dataKey="day" tickLine={false} axisLine={false} tick={{ fill: "#94a0b8", fontSize: 12 }} />
        <YAxis
          tickLine={false}
          axisLine={false}
          tick={{ fill: "#94a0b8", fontSize: 12 }}
          tickFormatter={(v) => (v >= 1_000_000 ? `${(v / 1_000_000).toFixed(0)}M` : `${v}`)}
          width={44}
        />
        <Tooltip
          cursor={{ fill: "#eef1f8" }}
          contentStyle={{ borderRadius: 16, border: "none", boxShadow: "0 8px 24px rgba(30,45,90,0.12)" }}
          formatter={(value: number) => [
            `${formatNumber(value, locale)} ${t.common.sum}`,
            t.expenses.amount,
          ]}
          labelFormatter={(_, payload) => payload?.[0]?.payload?.fullDay ?? ""}
        />
        <Bar dataKey="sum" fill="#f97316" radius={[8, 8, 0, 0]} maxBarSize={28} />
      </BarChart>
    </ResponsiveContainer>
  );
}
